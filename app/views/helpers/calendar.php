...
*/
	...
	function calendar($year = '', $month = '', $data = '', $base_url ='', $isAdmin = false)
		{	
			if($isAdmin){
			...
			if($isAdmin){
				$str .= $this->Html->link(__('prev', true), '/'.$prefix.'/events/index/' . $prev_year . '/' . $prev_month);
			}else{
				$str .= $this->Html->link(__('prev', true), 'events/' . $prev_year . '/' . $prev_month);
			} 
			
			$str .= '</th><th colspan="5">' . ucfirst($month) . ' ' . $year . '</th><th class="cell-next">';
			
			if($isAdmin){
				$str .= $this->Html->link(__('next', true), '/'.$prefix.'/events/index/' . $next_year . '/' . $next_month);
			}else{
				$str .= $this->Html->link(__('next', true), 'events/' . $next_year . '/' . $next_month);
			} 
								...
								for($i = 0; $i < 7; $i ++)
									{
										...
										if(isset($data[$day]))
											{
												foreach($data[$day] as $event_day){
													$cell .= $this->Html->link($event_day['title'], array('action' => $action, $event_day['id']), array('title' => __($event_day['title'], true)))."<br />";
												}
